...
class TryExcept:
    """Decorator class to handle exceptions in functions."""

    def __init__(self, msg="Exception occurred"):
        self.msg = msg
...
if platform.system() == "Windows":
    for fn in LOGGER.info, LOGGER.warning:
        setattr(LOGGER, fn.__name__, fn)  # safe logging without emojis
...
@TryExcept("Error in check_git_status")
...
def check_version(current="0.0.0", minimum="0.0.0", name="version ", pinned=False, hard=False, verbose=False):
    ...
    if hard:
        assert result, colorstr(s)  # assert min requirements met
